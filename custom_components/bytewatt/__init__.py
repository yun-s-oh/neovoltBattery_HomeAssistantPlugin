...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    recovery_options = {
        CONF_RECOVERY_ENABLED: options.get(
            CONF_RECOVERY_ENABLED, DEFAULT_RECOVERY_ENABLED
        ),
...
async def register_battery_services(
    hass: HomeAssistant, client: ByteWattClient, coordinator=None
):
    ...
    async def handle_force_reconnect(call: ServiceCall):
        ...
        for entry_id, entry_data in hass.data[DOMAIN].items():
            if "coordinator" in entry_data:
                ...
                _LOGGER.info(
                    f"Forcing recovery for ByteWatt integration (entry_id: {entry_id})"
                )
                try:
                    ...
                    reconnected = True
                    _LOGGER.info(
                        f"Recovery process completed for ByteWatt integration (entry_id: {entry_id})"
                    )
                except Exception as err:
                    _LOGGER.error(
                        f"Failed to recover ByteWatt integration (entry_id: {entry_id}): {err}"
                    )
    ...
    async def handle_health_check(call: ServiceCall):
        ...
        if entry_id:
            ...
            if (
                ...
                and "coordinator" in hass.data[DOMAIN][entry_id]
            ):
                coordinator = hass.data[DOMAIN][entry_id]["coordinator"]
                results[entry_id] = await coordinator.run_health_check()
            else:
                _LOGGER.error(
                    f"No ByteWatt integration found with entry_id: {entry_id}"
                )
        else:
    ...
    async def handle_toggle_diagnostics(call: ServiceCall):
        ...
        if results:
            ...
            message += (
                "Enabled"
                if list(results.values())[0].get("diagnostics_mode", False)
                else "Disabled"
            )
            try:
                await hass.components.persistent_notification.async_create(
    ...
    hass.services.async_register(
        ...
        schema=vol.Schema(
            {
                vol.Required(ATTR_MINIMUM_SOC): vol.All(
    ...
    hass.services.async_register(
        ...
        schema=vol.Schema(
            {
                vol.Required(ATTR_CHARGE_CAP): vol.All(
                    vol.Coerce(int), vol.Range(min=1, max=100)
                ),
            }
        ),
    ...
    hass.services.async_register(
        ...
        schema=vol.Schema(
            {vol.Optional("enable"): cv.boolean, vol.Optional("entry_id"): cv.string}
        ),
    )
