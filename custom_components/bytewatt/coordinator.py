...
class ByteWattDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        # Load options
        options = options or {}
        self._heartbeat_interval = options.get(
            CONF_HEARTBEAT_INTERVAL, DEFAULT_HEARTBEAT_INTERVAL
        )
        self._max_data_age = options.get(CONF_MAX_DATA_AGE, DEFAULT_MAX_DATA_AGE)
        self._stale_checks_threshold = options.get(
            CONF_STALE_CHECKS_THRESHOLD, DEFAULT_STALE_CHECKS_THRESHOLD
        )
        self._notify_on_recovery = options.get(
            CONF_NOTIFY_ON_RECOVERY, DEFAULT_NOTIFY_ON_RECOVERY
        )
        self._diagnostics_mode = options.get(
            CONF_DIAGNOSTICS_MODE, DEFAULT_DIAGNOSTICS_MODE
        )
        self._auto_reconnect_time = options.get(
            CONF_AUTO_RECONNECT_TIME, DEFAULT_AUTO_RECONNECT_TIME
        )
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            if not self.circuit_breaker.can_execute():
                ...
                self.diagnostic_service.log_diagnostic(
            ...
            try:
                if self.client.api_client.has_fresh_settings_update():
                    _LOGGER.debug(
                        "Skipping battery settings fetch - fresh update in progress"
                    )
                else:
            ...
            if battery_data and "soc" in battery_data:
                ...
                self._recovery_attempts = (
                    0  # Reset recovery attempts on successful update
                )

                self.diagnostic_service.log_diagnostic(
                    "data_update", {"type": "battery_data", "result": "success"}
                )
                ...
                if not battery_data:
                    error_msg = (
                        "Received empty response from API and no cached data available."
                    )
                ...
                self.diagnostic_service.log_diagnostic(
                    "data_update",
                    {"type": "battery_data", "result": "failure", "error": error_msg},
                )
                raise UpdateFailed(error_msg)
            else:
                _LOGGER.warning(
                    "Using cached battery data due to API error or invalid data received"
                )
                if not battery_data:
                    _LOGGER.debug("API returned empty response.")
                elif "soc" not in battery_data:
                    _LOGGER.debug(
                        f"API returned invalid data, missing 'soc'. Keys: {list(battery_data.keys())}"
                    )

                self.diagnostic_service.log_diagnostic(
                    "data_update",
                    {"type": "battery_data", "result": "fallback_to_cache"},
                )
            ...
            if self._last_battery_data:
                ...
                if self._last_successful_update:
                    age_seconds = (
                        request_start_time - self._last_successful_update
                    ).total_seconds()
                ...
                if self._notify_on_recovery:
                    try:
                        ...
                        _LOGGER.error(
                            f"Could not create error notification: {notification_error}"
                        )
    ...
    async def start_auto_reconnect(self) -> None:
        ...
        _LOGGER.info(
            f"Automatic reconnect scheduled every {AUTO_RECONNECT_INTERVAL_HOURS} hours"
        )
        ...
        if hasattr(self, "_auto_reconnect_time") and self._auto_reconnect_time:
            try:
                ...
                if reconnect_time:
                    ...
                    _LOGGER.info(
                        f"Auto reconnect time configured for {self._auto_reconnect_time}"
                    )
            except Exception as err:
    ...
    async def _handle_auto_reconnect(self, _now: Optional[datetime] = None) -> None:
        ...
        _LOGGER.info(
            f"Executing scheduled auto reconnect at {current_time.strftime('%H:%M:%S')}"
        )
        self.diagnostic_service.log_diagnostic(
            "auto_reconnect", {"trigger": "scheduled", "time": current_time.isoformat()}
        )
    ...
    async def _perform_recovery(self, is_scheduled: bool = False) -> None:
        ...
        _LOGGER.warning(
            f"Performing ByteWatt integration {recovery_type} recovery (attempt {self._recovery_attempts})"
        )
        ...
        try:
            ...
            self.diagnostic_service.log_diagnostic(
                "recovery_result",
                {"success": True, "timestamp": success_timestamp.isoformat()},
            )
    ...
    def _check_network(self) -> Dict[str, Any]:
        ...
        if hasattr(self.client, "api_client") and hasattr(
            self.client.api_client, "base_url"
        ):
        ...
        try:
            ...
            result["dns_check"] = {"success": False, "domain": domain, "error": str(e)}
    ...
    async def _reset_client(self) -> None:
        ...
        try:
            ...
            self.diagnostic_service.log_diagnostic(
                "client_reset",
                {"success": True, "timestamp": dt_util.utcnow().isoformat()},
            )
        except Exception as err:
    ...
    async def run_health_check(self) -> Dict[str, Any]:
        ...
        self.diagnostic_service.log_diagnostic(
            "health_check", {"timestamp": health_timestamp.isoformat()}
        )

        # Check network connectivity
        try:
            health_result["network_checks"] = await self.hass.async_add_executor_job(
                self._check_network
            )
        except Exception as err:
            health_result["network_checks"] = {"success": False, "error": str(err)}
        ...
        try:
            ...
            health_result["authentication"] = {"success": False, "error": str(err)}
        ...
        try:
            ...
            api_checks["battery_endpoint"] = {"success": False, "error": str(err)}
        ...
        # Add overall status
        if (
            health_result["network_checks"].get("ping_check", {}).get("success", False)
            and health_result["authentication"].get("success", False)
            and api_checks.get("battery_endpoint", {}).get("success", False)
        ):
