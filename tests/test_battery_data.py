...
def login(
    username: str, password: str, base_url: str = "https://monitor.byte-watt.com"
) -> str:
    ...
    try:
        ...
        if result.get("code") != 0 and result.get("code") != 200:
            logger.error(
                f"Login failed with code {result.get('code')}: {result.get('msg')}"
            )
            return None
...
def get_battery_data(
    token: str, station_id: str = None, base_url: str = "https://monitor.byte-watt.com"
) -> dict:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to get battery data with status {response.status_code}: {response.text}"
            )
            return None
        ...
        if result.get("code") != 0 and result.get("code") != 200:
            logger.error(
                f"Failed to get battery data with code {result.get('code')}: {result.get('msg')}"
            )
            return None
