...
def test_api_login(username, password, base_url="https://monitor.byte-watt.com"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            ):  # Only print response text for non-server errors
...
def test_api_login_fallback(
    username, password, base_url="https://monitor.byte-watt.com"
):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0 or result.get("code") == 200:
                ...
                logging.warning(
                    f"Fallback login failed with code {result.get('code')}: {result.get('msg')}"
                )
        else:
            logging.error(
                f"Fallback API request failed with status {response.status_code}"
            )
            if (
                response.status_code < 500
            ):  # Only print response text for non-server errors
